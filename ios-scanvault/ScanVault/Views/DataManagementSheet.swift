...
struct DataManagementSheet: View {
    ...
    private var deletionSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(spacing: 12) {
                Text("Permanently deletes every cloud record, local file, staged sync upload, signature asset, anonymous vault identifier and telemetry counter tied to this device's ScanVault install.")
                    .font(.system(size: 13))
                    .foregroundStyle(Theme.textSecondary)
                    .frame(maxWidth: .infinity, alignment: .leading)
